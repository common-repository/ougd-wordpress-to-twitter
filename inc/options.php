...
function wp_ozh_yourls_admin_notice() {
	global $plugin_page;
	if( $plugin_page == 'ougd' ) {
		$message = '<strong>ou.gd - WordPress to Twitter</strong> configuration incomplete';
	} else {
		$url = menu_page_url( 'ougd', false );
		$message = 'Please configure <strong>ou.gd - WordPress to Twitter</strong> <a href="'.$url.'">settings</a> now';
	}
	$notice = <<<NOTICE
...
function wp_ozh_yourls_add_page() {
	...
	$page = add_options_page('ou.gd: WordPress to Twitter', 'ou.gd', 'manage_options', 'ougd', 'wp_ozh_yourls_do_page');
...
function wp_ozh_yourls_add_css_js_plugin() {
	...
	wp_enqueue_script('yourls_js', $plugin_url.'res/ougd.js');
	wp_enqueue_script('wp-ajax-response');
	wp_enqueue_style('yourls_css', $plugin_url.'res/ougd.css');
}
...
function wp_ozh_yourls_do_page() {
	...
	<h2>ou.gd - WordPress to Twitter</h2>
	
	<div id="y_logo">
		<div class="y_logo">
			<a href="http://ou.gd/"><img src="<?php echo $plugin_url; ?>/res/logo.png"></a>
		</div>
		<div class="y_text">
			<p><a href="http://ou.gd/">ou.gd</a> is a free URL shortener service you can use to shorten any link and share it.</p>
			<p>This plugin is a bridge between <a href="http://ou.gd">ou.gd</a>, <a href="http://twitter.com/">Twitter</a> and your blog will tap into the ou.gd API to generate a short URL for it, and will then tweet it.</p>
		</div>
	</div>
	...
	<option value="yourls" <?php selected( 'yourls', $ozh_yourls['service'] ); ?> >ou.gd API</option>
	</select>
	
	<?php $hidden = ( $ozh_yourls['service'] == 'yourls' ? '' : 'y_hidden' ) ; ?>
	<div id="y_show_yourls" class="<?php echo $hidden; ?> y_service y_level2">
		<label for="y_location">Please choose a server</label>
		...
		<option value="remote" <?php selected( 'remote', $ozh_yourls['location'] ); ?> >primary server</option>
		</select>
		
		<?php $hidden = ( $ozh_yourls['location'] == 'local' ? '' : 'y_hidden' ) ; ?>
		<div id="y_show_local" class="<?php echo $hidden; ?> y_location y_level3">
			<label for="y_path">Path to ou.gd <tt>config.php</tt></label> <input type="text" class="y_longfield" id="y_path" name="ozh_yourls[yourls_path]" value="<?php echo $ozh_yourls['yourls_path']; ?>"/> <span id="check_path" class="yourls_check button">check</span><br/>
			<em>Example:</em> <tt>/home/you/site.com/includes/config.php</tt>
		</div>
		
		<?php $hidden = ( $ozh_yourls['location'] == 'remote' ? '' : 'y_hidden' ) ; ?>
		<div id="y_show_remote" class="<?php echo $hidden; ?> y_location y_level3">
			<label for="y_url">URL to API</label> <input type="text" id="y_url" class="y_longfield" name="ozh_yourls[yourls_url]" value="http://ou.gd/api.php"/> <span id="check_url" class="yourls_check button">check</span><br/>
			<em>If nothing is entered put:</em> <tt>http://ou.gd/api.php</tt><br/>
			<!-- <label for="y_yourls_login">Login</label> <input type="text" id="y_yourls_login" name="ozh_yourls[yourls_login]" value="<?php echo $ozh_yourls['yourls_login']; ?>"/><br/>
			<label for="y_yourls_passwd">Password</label> <input type="password" id="y_yourls_passwd" name="ozh_yourls[yourls_password]" value="<?php echo $ozh_yourls['yourls_password']; ?>"/><br/> -->
		</div>
		<?php
	...
	$blogdesc = urlencode( trim( get_bloginfo( 'description' ), '.' ).'. Powered by Logic Networks.' );
	...
	<p>Need to register? <a href="http://dev.twitter.com/apps/new">Register an Application</a> and fill the form to get your keys and tokens</p>
	<ul id="appdetails">
...
function wp_ozh_yourls_addbox() {
	...
	add_meta_box( 'yourlsdiv', 'ou.gd', 'wp_ozh_yourls_drawbox', $post_type, 'side', 'default' );
...
function wp_ozh_yourls_drawbox( $post ) {
	...
	if ( $status != 'publish' ) {
		echo '<p>Depending on <a href="options-general.php?page=ougd">configuration</a>, a short URL will be generated and/or a tweet will be sent.</p>';
		return;
	}
	...
	<input type="hidden" id="ougd_shorturl" value="'.$shorturl.'" />
	...
	<p>You can click Reset to generate another short URL if you picked another URL shortening service in the <a href='options-general.php?page=ougd'>plugin options</a></p>";
