...
function wp_ozh_yourls_reset_url() {
	...
	$old_shorturl = $_POST['ougd_shorturl'];
	delete_post_meta($post_id, 'ougd_shorturl');
	$shorturl = wp_ozh_yourls_geturl( $post_id );

	if ( $shorturl ) {
		$result = "New short URL generated: <a href='$shorturl'>$shorturl</a>";
		update_post_meta($post_id, 'ougd_shorturl', $shorturl);
	} else {
...
function wp_ozh_yourls_get_new_short_url( $url, $post_id = 0, $keyword = '', $title = '' ) {
	...
	if( $post_id ) {
		...
		update_post_meta( $post_id, 'ougd_shorturl', '' ); // temporary empty title to avoid loop on creating short URL
	}
...
function wp_ozh_yourls_plugin_actions($links) {
	$links[] = "<a href='options-general.php?page=ougd'><b>Settings</b></a>";
	return $links;
}
